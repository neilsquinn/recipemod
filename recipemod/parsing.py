...
class MicrodataParser:
    ...
    def extract_text_props(self, prop_name, single_result=False, clean=True):
        ...
        texts = [' '.join([
                             token.strip() 
                             for token in self.get_attr_text(tag).split()
                           ])
                 for tag in tags]
        if clean:
    ...
    def get_recipe(self):
        ...
        recipe['description'] = self.extract_text_props('description', single_result=True)
...
class LDJSONParser:
    ...
    def extract_ldjson_recipe(self) -> list:
        ...
        for tag in self.script_tags:
            print('tag is:', tag)
            print('type of tag text is:', type(tag.text), 'tag text is:', tag.text, 'tag string is:', tag.string)
            print('where was it?')
    ...
    def get_recipe(self):
        ...
        recipe['ingredients'] = [clean_text(item) 
                                 for item in ldjson_recipe.get('recipeIngredient')]
