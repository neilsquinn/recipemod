...
function RecipeView(props) {
  ...
  useEffect(() => {
    ...
    axios
      .get(`/api/recipes/${props.recipeId}`)
      .then((resp) => {
  ...
  useEffect(() => {
    ...
    if (editSaved) {
      ...
      axios
        .put(`/api/recipes/${props.recipeId}`, { recipe: editedRecipe })
        .then((resp) => {
  ...
  function handleDelete(event) {
    ...
    axios
      .delete(`/api/recipes/${props.recipeId}`)
      .then((resp) => history.push("/"))
      .catch((error) => {
        if (error.response.status == 404) {
          alert("Could not delete recipe as it was not found.");
        } else {
        ...
        return (
          <div>
            {instructions.sections.map((section) => {
              return (
                <div>
                  <h6 className="initialism">{section.name}</h6>
                  <ol>
        ...
        return (
          <textarea
            ...
            onChange={() => {
              setEditedRecipe({
                ...recipe,
                instructions: { type: "one_step", step: event.target.value },
              });
            }}
          />
        );
        ...
        return (
          <div>
            {instructions.sections.map((section, sectionIndex) => {
              ...
              return (
                <Fragment>
                  <label for={tagName} className="initialism">
                    {section.name}
                  </label>
                  <textarea
                    className="form-control"
  ...
  function getKeywordCategoryValues() {
    let info = [];
    recipe.category
      ? info.push({ label: "Category", value: recipe.category.join(", ") })
      : null;
    recipe.keywords
      ? info.push({ label: "Keywords", value: recipe.keywords.join(", ") })
      : null;
    return info;
  }
        ...
        <div className={recipe.image_url ? "col-sm-7" : "col-sm-12"}>
          ...
          {recipe.description ? (
            <div id="description" className="collapse show">
              <p className="font-italic">"{recipe.description}"</p>
            </div>
          ) : null}
          <InfoPanel
            info={[{ label: "Yield", value: recipe.yield }].concat(
              getTimeLabels(recipe.times)
            )}
          />
